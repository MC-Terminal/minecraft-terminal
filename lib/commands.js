...
const { parseStr } = require('./utils');
const parseCoords = require('./parseCoords');
const { parseVar } = require('./utils');
...
commands.optcmd = async (commandArr, options = { type: undefined }) => {
	...
	const aliasKeys = utils.toLowerCaseArr(Object.keys(commandAliases));
	const aliasValues = utils.toLowerCaseArr(Object.values(commandAliases));
...
function parseInput (str) {
	...
	for (let a = 0; a < out.length; a++) {
		...
		out[a] = e.replace(quotes + quoted + quotes, quoted);
	}
