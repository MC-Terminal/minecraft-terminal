...
const { parseStr, parseVar, matchEq, toLowerCaseArr, includes } = require('./utils');
...
commands.digOnChange = (x, y, z) => {
	if (isNaN(x) || isNaN(y) || isNaN(z)) {
		info(commandUsage.digOnPlace || 'USAGE');
		return;
	}
	if (commands.tmp.botLooking === true) {
		warn('Cannot use this command while player is attacking/looking at something.');
	...
	commands.digOnChange.coords = `${x}, ${y}, ${z}`;
	bot.on(`blockUpdate:(${x}, ${y}, ${z})`, commands.digOnChange.onBlockUpdate);

	success('Started');
};
commands.digOnChange.onBlockUpdate = (oldBlock, newBlock) => {
...
commands.stopDigOnChange = (x, y, z) => {
	bot.off(`blockUpdate:(${commands.digOnChange.coords})`, commands.digOnChange.onBlockUpdate);
