...
const commandDescriptions = {
	...
	digOnPlace: 'Waits for a block update and digs the block in the specified coords',
	stopDigOnPlace: 'Stops digOnPlace',
...
const commandUsage = {
	...
	dig: 'Usage: .dig <X> <Y> <Z>',
	stopdig: null,
	digOnPlace: 'Usage: .digOnPlace <X> <Y> <Z>',
	stopDigOnPlace: null,
...
commands.dig = async (x, y, z) => {
	...
	try {
		...
		let moreInfo = '';
		if (err.message === 'Block not in view') {
			moreInfo = '.\nTry moving closer to the block';
		} else if (err.message === 'dig was called with an undefined or null block') {
...
commands.digOnPlace = (x, y, z) => {
	if (isNaN(x) || isNaN(y) || isNaN(z)) {
		info(commandUsage.digOnPlace);
		return;
	}
	if (commands.tmp.botLooking === true) {
		warn('Cannot use this command while player is attacking/looking at something.');
	...
	commands.digOnPlace.coords = `${x}, ${y}, ${z}`;
	bot.on(`blockUpdate:(${x}, ${y}, ${z})`, commands.digOnPlace.onBlockUpdate);

	success('Started');
};
commands.digOnPlace.onBlockUpdate = (oldBlock, newBlock) => {
	if (newBlock.diggable === false) {
		return;
	}

	commands.dig(newBlock.position.x, newBlock.position.y, newBlock.position.z);
};

commands.stopdigOnPlace = (x, y, z) => {
	bot.off(`blockUpdate:(${commands.digOnPlace.coords})`, commands.digOnPlace.onBlockUpdate);
